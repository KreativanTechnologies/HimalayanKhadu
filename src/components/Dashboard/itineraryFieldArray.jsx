        ...
        <div key={day.id} className="space-y-4 border p-4 rounded-md">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="space-y-2">
              ...
              <ActivitiesFieldArray name={`${name}.${dayIndex}.todayActivities`} control={control} form={form} />
            </div>
...
function ActivitiesFieldArray({ name, control, form }) {
  const {
    fields: activities,
    append,
    remove,
    ...
    <div className="space-y-2">
      {activities.map((activity, activityIndex) => (
        <div key={activity.id} className="flex items-center gap-2">
          ...
          />
          <Button type="button" variant="outline" size="icon" onClick={() => append("")}>
            <PlusCircle className="h-4 w-4" />
            <span className="sr-only">Add activity</span>
          </Button>
          {activityIndex > 0 && (
            <Button type="button" variant="destructive" size="icon" onClick={() => remove(activityIndex)}>
